            ...
            ZStack(alignment: .topTrailing) {
                ...
                if unreadCount > 0 {
                    Circle()
                        ...
                        .overlay(Circle().stroke(Color.miraclesBackground, lineWidth: 1.5))
                        .offset(x: 1, y: -1)
                }
            }
        }
    ...
    private var emptyView: some View {
        VStack(spacing: 10) {
            Image(systemName: "tray")
                ...
                .foregroundStyle(Color.miraclesSecondary.opacity(0.4))
            Text("No notifications")
                .font(.system(size: 14, design: .serif))
                .foregroundStyle(Color.miraclesSecondary)
            Text("Boops and weekly prompts will land here.")
    ...
    private func row(_ entry: InboxEntry) -> some View {
        Button {
            ...
            HStack(alignment: .top, spacing: 12) {
                Circle()
                    .fill(entry.isRead ? Color.clear : Color.miraclesAccent)
                    .frame(width: 8, height: 8)
                    .padding(.top, 7)
